...
class Gambit_Pro_Header_Bar {
	...
	static function nav_menu_social_icons( $item_output, $item, $depth, $args ) {
		...
		// Get SVG.
		$svg = apply_filters( 'gambit_pro_get_social_svg', self::get_social_svg( $icon ), $item_output );
	...
	static function get_social_svg( $icon = null ) {
		...
		$svg .= ' <use xlink:href="' . GAMBIT_PRO_PLUGIN_URL . 'assets/icons/social-icons.svg?ver=20240124#icon-' . esc_html( $icon ) . '"></use> ';
		$svg .= '</svg>';
	...
	static function supported_social_icons() {
		...
		return apply_filters( 'gambit_pro_supported_social_icons', $supported_social_icons );
	}
